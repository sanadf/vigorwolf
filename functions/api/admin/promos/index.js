...
import { serializePromo, normalizeCode, PROMO_TYPES } from "../../_lib/promo.js";
  ...
  try {
    const res = await env.DB.prepare(
      `INSERT INTO promo_codes
        (code, campaign_name, influencer_name, description, discount_type, discount_value, active,
         starts_at, expires_at, max_uses, per_customer_limit, min_order_amount, max_discount_amount,
         product_ids, first_order_only)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`
    ).bind(
      code, String(b.campaignName || ""), String(b.influencerName || ""), String(b.description || ""),
      type, Number(b.discountValue) || 0, b.active ? 1 : 0,
      String(b.startsAt || ""), String(b.expiresAt || ""),
      parseInt(b.maxUses, 10) || 0, parseInt(b.perCustomerLimit, 10) || 0,
      Number(b.minOrderAmount) || 0, Number(b.maxDiscountAmount) || 0,
      productIds, b.firstOrderOnly ? 1 : 0
    ).run();
